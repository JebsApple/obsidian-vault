...
| Scroll de brillo "afectaba" los números del módulo CPU | el script hacía `pkill -RTMIN+1 waybar` por tick → refrescaba TODOS los módulos `signal: 1` (incl. `custom/resources`); + spawns de Python subían el CPU real mostrado | quitar el `pkill` del scroll (el `%` se refresca por `interval: 1`) + cachear la detección de monitor 500 ms. **Señal RTMIN+N es compartida: refresca todos los módulos con ese `signal`** |
| hypr-audio no se abría al hacer clic derecho | `_gdk_monitor_for_cursor()` no existía en `hypr-audio` (copia de `hypr-player` que sí la tiene), y faltaban `_tick_vol`/`_tick_outs` (referenciados en `GLib.timeout_add` pero nunca definidos) | reemplazar por `_cursor_target()` existente + definir los timer callbacks. **Regla: al copiar código GTK con timers, verificar que los callbacks referenciados existan** |
